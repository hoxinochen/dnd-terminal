...
- **当前状态：** v0.5.0 Archived — Local Private；v0.6.0 ABC Approved/Frozen
- **当前交付：** `v0.6.0 — DM 裁定驱动的复活、转生与受控不死生物关联`
- **当前门禁：** Awaiting Explicit Implementation Authorization
- **Release / Technical Sync：** v0.5.0 Review、Local Private Release 与 Lightweight Archive 已获批完成；当前本地 Git 视图中 `main` 与 `origin/main` 均为 `ab82faf`，已包含产品、Authority 镜像与活文档治理技术同步；部署或公开发布仍未授权
...
User 于 `2026-08-31` 正式命名并批准冻结 `v0.6.0` ABC。该交付把 v0.5.0 的四张死亡后结果卡收敛为“恢复原身体”“以新身体或新形态继续冒险”“制造受控不死生物”三张卡；普通新身体、不死生物形态与其他自定义形态在第二张卡内并列，旧 v0.5.0 S2C 事件继续兼容。回生术、死者复活、复生术、完全复生术、转生术、克隆术、活化死尸与唤起亡灵只提供非阻塞提示和推荐结果，DM 裁定仍为最终 Authority。Implementation 尚未授权。
...
全部项目文档的 canonical Authority 只在 Obsidian；Workspace 的 `authority-mirror/` 与根 README 是单向技术投影。v0.5.0 产品实现为 `370b92a`，Authority 镜像为 `2f897a6`，活文档治理同步为 `ab82faf`；当前 `main` 与 `origin/main` 均指向 `ab82faf`。这些 post-archive 技术同步不改变冻结的 Local Private Release/Archive 身份；本次 v0.6.0 文档改动尚未 Commit/Push，部署与公开发布仍未授权。
...
对于已归档的 `v0.4.0` 与 `v0.5.0`：两者均继续采用 Downstream Documentation Schema Core `schema-v0.3.0`，兼容 Minimal Governance Framework `schema-v0.2.0`；因各自只有一个主实施 Slice、没有跨 Slice Exit Gate，Milestone Documentation Profile 均为 `Not Applicable`。`v0.6.0` 同样冻结为单一主 Slice，Milestone Documentation Profile `Not Applicable`；其 Archive Contract/Profile 尚未选择。历史 Archive 选择均不得自动继承。
...
v0.4.0 与 v0.5.0 均已完成其获授权的 Review、Local Private Release 和 Lightweight Archive。v0.6.0 ABC 已批准冻结，下一门禁仅为 User 明确授权 Implementation；ABC 不推导 Implementation、Review、Release、Archive、Commit、Push、部署、公开发布或再分发权限。
