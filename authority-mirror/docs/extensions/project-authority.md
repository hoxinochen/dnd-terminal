...
- **Status：** Stable Local Extension；v0.6.0、v0.7.0 与 v0.7.1 Archived — Local Private
...
- `v0.7.1` 于 `2026-09-10` 获 User 批准版本身份、范围文档和主线 merge。它是 v0.7.0 归档后的独立 UX 适配交付，候选 commit 为 `58ebd92`，main 集成 commit 为 `6ae7636`；ABC、Implementation、Testing、Review 与 Gemini/AG/Codex 协作记录已建立，主线回归 26/26 通过。User 随后授权 Local Private Release 与 Archive，发布包、内容清单、解压复验、包内 26/26 回归、51 文件快照和八项 Archive Gate 均已完成，证据见 `version-work/v0.7.1/RELEASE_NOTES.md` 与 `version-work/v0.7.1/archive/ARCHIVE_SUMMARY.md`。它不改写 v0.7.0 冻结材料；Push、部署和公开发布仍未授权。
- Archive Contract/Profile 只能在具体交付获得 Archive 方向授权并完成适用性选择后采用；历史版本的选择不得自动继承。
- 当前交付为已归档的 `v0.7.1 — 统一战斗工作台 UX 适配`；`v0.7.0` 是其历史技术与治理基线。v0.7.1 的下一门禁为 Push，仍需 User 单独授权。
...
- 当前门禁为 v0.7.1 Main Integration、主线回归、Local Private Release 和 Local Private Archive 已完成。Push、部署和公开发布仍未授权。

当前下一门禁是 v0.7.1 的 Push 方向决定；该门禁尚未授权。Backlog 的 P1、AI 推进建议和 Roadmap 方向不得扩张已归档的 v0.7.1 范围。
