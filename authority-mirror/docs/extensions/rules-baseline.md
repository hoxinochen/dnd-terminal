...
- **Status：** Approved/Frozen historical Entries；v0.6.0 复活、转生与受控不死生物提示 Entries implemented as non-blocking local guidance，User Human Acceptance Passed，Archived — Local Private
- **Owner：** User
...
- **Status：** `Approved / Frozen for v0.6.0 non-blocking guidance / Implemented / User Human Acceptance Passed / Review Approved — User / Archived — Local Private`。中文翻译、出版身份、许可与再分发权继续为 `unknown`。
