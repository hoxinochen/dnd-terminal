...
function workspaceNavButton(workspace){
  const glyphMap={battle:'台',map:'图',characters:'人',library:'库',log:'志',dice:'骰',settings:'设'};
  const glyph=glyphMap[workspace.id]||(workspace.label?workspace.label[0]:'●');
...
function workspaceChromeMarkup(workspaceId, content){
  if (['battle', 'map'].includes(workspaceId)) {
    return `<section class="workspace workspace-${workspaceId} workspace-unified-workbench" data-workspace-density="${uiPreferences.density}"><div class="workspace-content" data-workbench-content>${content}</div></section>`;
  }
...
function preparationMapView(){return unifiedWorkbenchView();}
...
function mapViewV2(){return unifiedWorkbenchView();}
...
function render(){
  ...
  if(layout){
    layout.classList.toggle('rail-collapsed',!!uiPreferences.workbenchRailCollapsed);
    const railBtn=shell.querySelector('[data-rail-toggle].rail-collapse-btn');
    if(railBtn){
